...
param_struct.num_links = 180;
param_struct.obs_x = [6]; %[2 7];
param_struct.obs_y = [0]; %[3 1];
param_struct.obs_r = [2]; %[1 2]; 

%%%% Gradient descent parameters %%%%
param_struct.beta = 0.05; %Original: 0.005 %WHAT DOES THIS DO?
param_struct.gamma = 0.50; %Original: 0.9 %WHAT DOES THIS DO? 
param_struct.max_iter = 100;
param_struct.min_gradient = 1e-7;

param_struct.cost_coeff = 10;
param_struct.wiggle = 0; %says if we "wiggle" the start guess line. 0 = no, 1 = yes
param_struct.wiggle_mag = 10; %says the maximum amplitude of the wiggle

param_struct.length_multiplier = 10

function [x_list, y_list] = gen_shape_prediction(param_struct)
...
function full_length = find_lengths(vector, param_struct)
    ...
    for i = 1 : length(lengths)
        ...
        if(lengths(i) >0.05)
            lengths(i) = lengths(i)^param_struct.length_multiplier;
        end
...
[x, y] =  gen_shape_prediction(param_struct);
