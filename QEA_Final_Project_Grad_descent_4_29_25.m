...
param_struct.num_links = 100;
param_struct.obs_x = [6]; %[2 7];
param_struct.obs_y = [0]; %[3 1];
param_struct.obs_r = [1]; %[1 2]; 

%%%% Gradient descent parameters %%%%
param_struct.beta = 0.5; %Original: 0.005 %WHAT DOES THIS DO?
param_struct.gamma = 0.90; %Original: 0.9 %WHAT DOES THIS DO? 
param_struct.max_iter = 70;
param_struct.min_gradient = 1e-7;

% multiplier which says the multiplier on the cost if a VERTEX is within the
% circle. 
param_struct.cost_coeff = 100;
param_struct.wiggle = 0; %says if we "wiggle" the start guess line. 0 = no, 1 = yes
param_struct.wiggle_mag = 2; %says the maximum amplitude of the wiggle

%Parameters for attempting to constrain the length
param_struct.length_multiplier = 1;
param_struct.max_length_before_penalty = 0.25;
...
function full_length = find_lengths(vector, param_struct)
    ...
    for i = 1 : length(lengths)
        ...
        if(lengths(i) >param_struct.max_length_before_penalty)
            lengths(i) = lengths(i) * exp(param_struct.length_multiplier);
        end
...
function [x_list, y_list] = generate_shape_prediction(param_struct)
    ...
    for i = 1:int32(param_struct.max_iter/3):(length(otherVs(1, :)))
        ...
        plot(x_list, y_list, lineWidth = 1)
        a(count, 1) = i;
    ...
    x_list = V_list(1:2:(end-1), 1);
    y_list = V_list(2:2:end, 1);
    plot(x_list, y_list, LineWidth = 2)
