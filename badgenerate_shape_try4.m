...
param_struct.num_links = 80;
param_struct.obs_x = [6 1.5 10]; %[2 7];
param_struct.obs_y = [-1 -1.5 1.5]; %[3 1];
param_struct.obs_r = [3 1 1]; %[1 2]; 
...
param_struct.cost_coeff = 100; %70
param_struct.length_multiplier = 10; %100
...
function [Vopt, allVs] = run_gradient_descent(fun,V0,params)
    ...
    while n<max_iter && norm(G)>min_gradient
        ...
        % if(x <0.4)
        % 
        %     update = (update/norm(update))*0.4;% * (1-x);
        % end
